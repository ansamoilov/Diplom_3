...
def reset_password_page(drivers):
    ...
    reset_pages = {}
    for browser, driver in drivers.items():
        driver.get(forgot_password_url)  # Переходим на страницу восстановления пароля
        forgot_password_page = ForgotPasswordPage(driver)
        forgot_password_page.click_reset_button()  # Кликаем по кнопке "Восстановить"
        reset_pages[browser] = ResetPasswordPage(driver)  # Переходим на страницу сброса пароля
    return reset_pages
